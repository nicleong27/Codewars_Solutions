# Write a function that returns a (custom) FizzBuzz sequence of the numbers 
# 1 to 100.
...
# Thus, when the function is called without arguments, it will return the 
# classic FizzBuzz sequence up to 100:

# [ 1, 2, "Fizz", 4, "Buzz", "Fizz", 7, ... 14, "FizzBuzz", 16, 17, ... 98, 
# "Fizz", "Buzz" ]
...
def fizz_buzz_custom(string_one='Fizz', string_two='Buzz', num_one=3, num_two=5):
    default_lst = [i for i in range(1, 101)]
    
    for idx, num in enumerate(default_lst):
        if num % num_one == 0 and num % num_two == 0:
            default_lst[idx] = string_one + string_two
        elif num % num_one == 0:
            default_lst[idx] = string_one
        elif num % num_two == 0:
            default_lst[idx] = string_two
