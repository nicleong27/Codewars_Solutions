...
def stock_list(listOfArt, listOfCat):
    ...
    listOfArt_lst = [book.split() for book in listOfArt]
    d = {}
    
    for book in listOfArt_lst:
        if book[0] not in d:
            d[book[0]] = 0
        d[book[0]] = book[1]
    ...
    combined = tuple(zip(listOfCat, sums))
    output = []
    
    for idx, tup in enumerate(combined):
        output.append('(' + tup[0] + ' : ' + str(tup[1]) + ')')
