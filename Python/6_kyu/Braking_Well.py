...
# brakes to when it comes to a complete stop. It depends on the original speed 
# v and on the coefficient of friction mu between the tires and the road surface.
...
# Equalling E and W gives the braking distance: d1 = v*v / 2*mu*g where g is the 
# gravity of Earth and m the vehicle's mass.
...
# The first one is to calculate the total stopping distance in meters given 
# v, mu (and the reaction time t = 1).
...
# Don't truncate or round your results. See in "RUN SAMPLE TESTS" the function 
# assertFuzzyEquals or dotest-....
...
def dist(v, mu): 
    g = 9.81
    t = 1
    v_in_mps = v * 1000 * (1/3600)
    
    
    d1 = (v_in_mps**2)/(2 * mu * g)
    d2 = (v_in_mps) * t
    d = d1 + d2
    return d
