...
-- return all columns from the people table, and join to the sales table so that 
-- you can return the COUNT of all sales and RANK each person by their sale_count.
...
SELECT p.id, p.name, COUNT(s.id) AS sale_count, 
